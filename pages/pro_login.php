...
if (isset($_POST['register'])) {
    ...
    $admin_id    = intval($_POST['society']); // selected society
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        $insertQuery = "
            ...
            (full_name, email, phone, password, skill_type, other_skill, created_at)
            VALUES
            ('$full_name', '$email', '$phone', '$password', '$skill_type', '$other_skill', '$created_at')
        ";
        ...
        if (mysqli_query($conn, $insertQuery)) {
            ...
            // Map professional to selected society
            mysqli_query(
                $conn,
                "INSERT INTO admin_professional (admin_id, pro_id)
                 VALUES ($admin_id, $pro_id)"
            );
...
<!-- LOGIN FORM -->
<div class="auth-form-wrapper" id="Pro_login">
    <h2>Welcome Back</h2>
...
<div class="auth-form-wrapper hidden" id="registerForm">
    ...
    <form class="auth-form" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="tel" name="registerPhone" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="society" required>
                ...
                <?php
                $societies = mysqli_query($conn, "SELECT admin_id, admin_name FROM admin");
                while ($soc = mysqli_fetch_assoc($societies)) {
                    echo "<option value='{$soc['admin_id']}'>" .
                         htmlspecialchars($soc['admin_name']) .
                         "</option>";
                }
                ?>
            </select>
    ...
    <p class="auth-toggle">
        ...
        <a href="#" onclick="return toggleForm()">Sign in here</a>
    </p>
</div>
...
<script>
function toggleForm() {
    document.getElementById('Pro_login').classList.toggle('hidden');
